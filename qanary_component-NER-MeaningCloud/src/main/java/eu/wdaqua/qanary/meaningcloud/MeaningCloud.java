...
public class MeaningCloud extends QanaryComponent {
	...
	public QanaryMessage process(QanaryMessage myQanaryMessage) throws Exception {
		...
		QanaryUtils myQanaryUtils = this.getUtils(myQanaryMessage);
	      ...
	      HttpGet httpget = new HttpGet("https://api.meaningcloud.com/topics-2.0?key=57748bf27b54ff6a49d2a0947e15754c&of=json&lang=en&ilang=en&txt="+thePath+"&tt=e&uw=y");
    ...
    // String result = getStringFromInputStream(instream);
 	            ...
 	            if(response2.has("entity_list"))
 	            {
 	            	...
 	            	if(ents.length()!=0)
 	            	{
 	            	JSONObject test = (JSONObject) ents.get(0); 
 	            	logger.info("JA: {}", ents);
 	            	logger.info("JA: {}", test);
 	            	if(response2.has("variant_list"))
 	            	{
 	            	JSONArray jsonArray = (JSONArray) test.get("variant_list");
 	            	if(jsonArray.length()!=0)
 	            	{
 	            		for (int i = 0; i < jsonArray.length(); i++) {
 	            			JSONObject explrObject = jsonArray.getJSONObject(i);
 	            			int begin = Integer.parseInt((String) explrObject.get("inip"));
 	            			int end = Integer.parseInt((String)explrObject.get("endp"));
