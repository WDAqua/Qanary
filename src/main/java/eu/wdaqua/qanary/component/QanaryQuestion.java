...
public class QanaryQuestion<T> {
    ...
    public URI getUriTextualRepresentation() throws Exception {
        if (this.uriTextualRepresentation==null) {
            ...
            if (i > 1) {
                throw new Exception("More than 1 text representation (count: " + i + ") in graph " + this.getInGraph() + " at "
                        + this.getEndpoint());
            } else if (i == 0) {
                throw new Exception("No uriTextRepresentation available in graph " + this.getInGraph() + " at " + this.getEndpoint());
            }

            logger.info("uriTextRepresentation {} found in {} at {}", uriTextRepresentation, this.getInGraph(), this.getEndpoint());
    ...
    public String getTextualRepresentation() throws Exception {
        RestTemplate restTemplate = new RestTemplate();
        ResponseEntity<String> responseRaw = restTemplate.getForEntity(this.getUriTextualRepresentation()+QanaryConfiguration.questionRawDataUrlSuffix, String.class);
    ...
    public URI getUriAudioRepresentation() throws Exception {
        if (this.uriAudioRepresentation==null) {
            ...
            if (i > 1) {
                ...
                throw new Exception("No uriAudioRepresentation available in graph " + this.getInGraph() + " at " + this.getEndpoint());
            }

            logger.info("uriAudioRepresentation {} found in {} at {}", uriAudioRepresentation, this.getInGraph(), this.getEndpoint());
    ...
    public byte[] getAudioRepresentation() throws Exception {
        RestTemplate restTemplate = new RestTemplate();
        ResponseEntity<byte[]> responseRaw = restTemplate.getForEntity(this.getUriAudioRepresentation()+QanaryConfiguration.questionRawDataUrlSuffix, byte[].class);
