...
However, if you would like to see how QA systems are built using the Qanary framework, take a look at the [Qanary Question Answering components](https://github.com/WDAqua/Qanary-question-answering-components) and the examples in the [Wiki](https://github.com/WDAqua/Qanary/wiki).
...
The Qanary component template is a ready-to-use Spring Boot skeleton (Qanary message handling, triplestore access, REST endpoints and a Swagger/OpenAPI UI) that you extend to build your own component. A step-by-step guide is on the Wiki: [How do I create a new Qanary component?](https://github.com/WDAqua/Qanary/wiki/How-do-I-create-a-new-Qanary-component%3F).
...
The QALD evaluator is a client for a running Qanary Pipeline that measures its named-entity recognition and disambiguation quality against the [QALD benchmark](http://qald.sebastianwalter.org/) (Question Answering over Linked Data).


<a name="qaldnerddataset"></a>
### QALD annotated with named entities
[source](https://github.com/WDAqua/Qanary/tree/master/ICWE-results)

[QALD](http://qald.sebastianwalter.org/) questions annotated with named entities, used by the QALD evaluator above. The data lives in the [`ICWE-results`](https://github.com/WDAqua/Qanary/tree/master/ICWE-results) directory.
...
### Without creating Docker images 
 ...
 4. Install Stardog Triplestore (<http://stardog.com/>) and start it in the background. Create a database with the name _qanary_. All the triples generated by the components will be stored in the _qanary_ database. We use version 8 of Stardog. (Stardog is one option; the pipeline can also use a plain SPARQL endpoint, e.g. Virtuoso or the in-memory triplestore.)
...
```
   ...
   java -jar qa.pipeline-<version>.jar
```
